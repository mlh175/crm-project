...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#saveBundActivityBtn").click(function () {
			...
			//收集参数
			//获取列表中所有被选择的checked
			var checkdis = $("#tBody input[type='checkbox']:checked");
			//表单验证
			if (checkdis.size()==0) {
				alert("请选择要关联的市场活动")
				return
			...
			var ids=""
			$.each(checkdis,function (index,obj) {
				ids+="activityId="+this.value+"&"
			})
			ids+="clueId=${clue.id}"//activityId=XXX&activityId=XXX&activityId=XXX&clue.id=XXX
			//发送请求
			$.ajax({
...
<%--						</tr>--%>
			...
			<div>
				<a href="javascript:void(0);" style="text-decoration: none;" id="bundActivityBtn"><span class="glyphicon glyphicon-plus"></span>关联市场活动</a>
			</div>
		</div>
	</div>
