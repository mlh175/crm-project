...
<script type="text/javascript">
	...
	$(function(){
		...
		$(".myDate").datetimepicker({
			...
			todayBtn:true,//设置是否显示“今天”按钮，默认设置false
			clearBtn:true//设置是否设置“清空”按钮，默认是false
		});
		...
		$("#saveCreateActivityBtn").click(function () {
			...
			/*
			正则表达式：
			1，语言，语法：定义字符串的匹配模式，可以用来判断指定的集体字符串是否符合匹配模式
			2，语法通则：
		...
		//当市场活动主页面加载完毕，查询所有数据的第一页以及所有数据的总条数，默认每页显示十条
			queryActivityByConditionForPage(1,5);

		//给查询按钮添加单机事件
		$("#queryActivityBtn").click(function () {
			//查询所有符合条件数据的第一页和所有符合条件数据的总条数
			queryActivityByConditionForPage(1,$("#demo_pag1").bs_pagination('getOption','rowsPerPage'));
		})
			//给全选按钮添加单机事件
		$("#checkAll").click(function () {
			//如果全选按钮是全选状态，则列表中的checkbox都选中
			/*if (this.checked) {
				$("#tBody input[type='checkbox']").prop("checked",true)
			}else $("#tBody input[type='checkbox']").prop("checked",false);
			*/
			$("#tBody input[type='checkbox']").prop("checked",this.checked)
		})
			$("#tBody").on("click","input[type='checkbox']",function () {
				//如果列表中所有的checkbox都选中，则全选按钮全选中
				if ($("#tBody input[type='checkbox']").size()==$("#tBody input[type='checkbox']:checked").size()){
					$("#checkAll").prop("checked",true)
				}else {//如果列表中所有的checkbox至少一个未选中，则全选按钮也取消
		...
		if (window.confirm("确认删除么？")) {//阻塞函数
            var ids="";
            $.each(tBody,function () {//index:下标   obj:循环变量
                ids+="id="+this.value+"&";
            })
			ids=ids.substring(0,ids.length-1);//id=XXX&id=XXX&id=XXX
            alert(ids)
			//发送请求
            $.ajax({
		...
		$("#editActivityBtn").click(function () {
			//收集参数
			//获取列表中被选中的checkbox
			var body = $("#tBody input[type='checkbox']:checked");
			if (body.size()==0){
				alert("请选择要修改的市场活动")
				return
			}
		...
		$("#importActivityBtn").click(function (){
			var activityListName = $("#activityFile").val();
			var substr = activityListName.substr(activityListName.lastIndexOf(".")+1).toLocaleLowerCase();
			if (substr!="xls"){
				alert("只支持xls文件")
				return;
			}
			...
			$.ajax({
				...
				data:formData,
				type:'post',
				processData:false,//设置ajax向后台提交参数之前，是否把参数同意转成字符串
				contentType:false,//设置ajax向后提交数据之前，是否把所有的参数统一按urlencoded编码
				dataType:'json',
				success:function (data) {
					if (data.code=="1") {
						//提示成功导入条数
	...
	function queryActivityByConditionForPage(pageNo,pageSize) {
		...
		$.ajax({
			...
			success:function (data) {
				...
				//显示市场活动的列表
				//遍历activityList，拼接所有行的数据
				//index:遍历的下标，从0开始  obj:循环变量
				var htmlStr="";
				...
				$("#demo_pag1").bs_pagination({
					currentPage:pageNo,//当前页号currentPage:1,//当前页号
